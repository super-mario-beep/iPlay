...
class AddStreamToPlaylistSheetDl extends StatefulWidget {
  final String stream;
  AddStreamToPlaylistSheetDl({
    @required this.stream
  });

  @override
  _AddStreamToPlaylistSheetStateDl createState() => _AddStreamToPlaylistSheetStateDl();
}

class _AddStreamToPlaylistSheetStateDl extends State<AddStreamToPlaylistSheetDl> with TickerProviderStateMixin {
  @override
              ...
              itemBuilder: (context, index) {
                ...
                return Container(child:Row(
                  children: [
                    ...
                    GestureDetector(
                      onTap: () async {
                        int fun = _getFunctionJobPlaylist(context, playlist ,widget.stream);
                        List<String> songs = prefs.getSongsForPlaylist(playlist);
                        if(fun == 1){
                          print("Removing " + widget.stream + " from " + playlist);
                          songs.remove(widget.stream);
                          prefs.setSongsForPlaylist(playlist, songs);
                        }else if(fun == 2){
                          print("Adding " + widget.stream + " to " + playlist);
                        ...
                        setState(() {
                          _AddStreamToPlaylistSheetStateDl createState() =>
                              _AddStreamToPlaylistSheetStateDl();
                        });
                      },
                      child: Container(
                        decoration: BoxDecoration(
                        ...
                        child: Row(
                          children: [
                            Icon(!_isSongInPlaylist(context, widget.stream, playlist) ? Icons.add_rounded : Icons.remove,
                                color: Theme.of(context).iconTheme.color,
                                size: 18),
                            SizedBox(width: 4),
                            _getFunctionTextPlaylist(context, playlist, widget.stream),
                          ],
                        ),
                      ),
                    ),
  ...
  bool _isSongInPlaylist(BuildContext context, String songName, String playlistName){
  ...
  int _getFunctionJobPlaylist(BuildContext context, String playlist, String songName){
  ...
  Widget _getFunctionTextPlaylist(BuildContext context, String playlist, String songName){
    bool isDownloaded = _isSongDownloaded(context, songName);
    bool isInPlaylist = _isSongInPlaylist(context, songName, playlist);
    if(isDownloaded){
      if(isInPlaylist){
      ...
      actions: [
        TextButton(
            child: Text(
              Languages.of(context).labelCreate,
              style: TextStyle(
                  color: controller.text.isNotEmpty && controller.text.length > 2
